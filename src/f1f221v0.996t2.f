...
CCC                                                                      CCC
CCC   F1F220 version 0.996  -  April 6, 2021                             CCC
...
c         GEn = (1.5972*tau / (1.0+0.19655*tau)) * GD
     ...
     &      / (1.D0 + 14.720D0*tau + 24.200D0*tau**2 + 84.100D0*tau**3)
        ...
        GEn = GEn*((q2+1.0114 )/1.0072)**0.60581
        GMn = GMn/((q2+1.1834)/1.1834)**0.19768
...
CCCC  multiple times.
      ...
      logical doqe,off,first/.false./
c      INCLUDE 'parm.cmn'
      external off_mKP_fit

      f1dqe = f1d   !!! Pass QE values in and IN+QE out if doqe = .true.
      f2dqe = f2d
      fLdqe = fLd      
...
c      off = .true.
      ...
      q2t = q2
      if(q2.LT.q2min) q2t = q2min  !!! Hack to fix photoproduction
      
      x = q2t/(w2-m2+q2t)
      gamma2 = 1.+4.*m2*x*x/q2t
      t1 = gamma2*f2dqe-2.*x*f1dqe

      if(f2dqe.LT.0.0.OR.f1dqe.LT.0.0.OR.fLdqe.LT.0.0)
     &    write(34,*) w2,q2,f2dqe,f1dqe,fLdqe

      call SQESUB(w2,q2t,wfn,f2dqe,f1dqe,fLdqe,first)
     ...
     & 0.75000E-01,0.38012E+00,0.72000E+01,0.36163E+01,0.16026E+01,
     & 0.21697E+01,0.27424E+01,0.18635E+01,0.17007E+02,0.12018E+01,
     & 0.29600E+01,0.92838E+01,0.21386E+02,0.22213E+01,0.51569E+03,
     & 0.82835E+03,0.85402E+02,0.64661E+05,0.26191E+00,0.22097E+02,
     & 0.28088E+00,0.23009E+01,0.27122E+01,0.58290E+04,0.72381E+04,
     & 0.25871E+02,0.33579E+00,0.16460E+01,0.25215E+02,0.11386E+00,
     & 0.79310E-01,0.52104E+00,0.22614E+01,0.49169E+01,-.78524E-01,
     & 0.10827E-01,0.19782E+01,0.54078E+00,0.26027E+01,0.19898E-01,
     & 0.10186E+01,0.97493E+00,0.99250E+00,0.98113E+00,0.10425E+01,
     & 0.10154E+01,0.98069E+00,0.98176E+00,0.10402E+01,0.10018E+01,
     & 0.10146E+01,0.10114E+01,0.13326E+02,0.98252E+02,0.69272E+01,
     & 0.11834E+01,0.13642E+00,0.20884E+02,0.30189E+01,0.19768E+00,
     & 0.71813E-03,0.63214E+01,0.21399E+01,0.77741E+02,0.14788E+01,
     & 0.12655E+02,0.29691E+01,0.98035E+00,0.11913E+02,0.23614E+02,
     & 0.76568E+01,0.10114E+01,0.29579E-03,0.24176E+01,0.83203E+00,
     & 0.60581E+00,0.11823E+00,0.29571E+01,0.12019E+01,0.10000E+01,
     & 0.36439E+02,0.51543E+01,0.91411E+01,0.77592E+01,0.68505E-01,
     & 0.84495E+00,0.19410E+01,0.44998E+00,0.10015E+01,0.37921E+01 /
